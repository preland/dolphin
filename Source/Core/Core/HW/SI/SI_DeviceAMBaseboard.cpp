...
class JVSIOMessage
{
  public:
    ...
    {
      m_last_start = m_ptr;
      unsigned char hdr[3] = {0xe0, static_cast<unsigned char>(node), 0};
      m_csum = 0;
      addData(hdr, 3, 1);
    }
...
int CSIDevice_AMBaseboard::RunBuffer(u8* _pBuffer, int _iLength)
{
  ...
  SerialInterface::ISIDevice::RunBuffer(_pBuffer, _iLength);
...
#define ptr(x) _pBuffer[(p + x)^3]
          while (p < real_len+2)
          {
            ...
            case 0x31:
              ERROR_LOG_FMT(AMBASEBOARDDEBUG, "GC-AM: CMD 31 (UNKNOWN)");
              res[resp++] = 0x31;
              res[resp++] = 0x02;
              res[resp++] = 0x00;
              res[resp++] = 0x00;
              break;
            case 0x32:
              ERROR_LOG_FMT(AMBASEBOARDDEBUG, "GC-AM: CMD 32 (UNKNOWN)");
              res[resp++] = 0x32;
              res[resp++] = 0x02;
              res[resp++] = 0x00;
              res[resp++] = 0x00;
